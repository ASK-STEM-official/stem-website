    ...
    <SectionWrapper id="about">
      <div className="grid md:grid-cols-2 gap-20 items-center mb-24">
        <FadeInUp>
          ...
          <p className="text-gray-600 dark:text-white/60 leading-[1.8] text-base mb-6">
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {stemLetters.map((item) => (
              <div
                key={item.letter}
                className="text-center py-5 px-4 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-white/[0.03]"
              >
                <span className="text-2xl font-bold text-brand-600 dark:text-brand-400 font-mono">{item.letter}</span>
                <p className="text-sm text-gray-700 dark:text-white/80 font-medium mt-1">{item.word}</p>
                <p className="text-xs text-gray-400 dark:text-white/40 mt-0.5">{item.jp}</p>
              </div>
            ))}
          </div>
          <p className="text-gray-500 dark:text-white/50 text-sm mt-4 leading-relaxed">
