...
export default function HeroSection() {
  return (
    <section className="relative h-screen min-h-[600px] flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div
        className="absolute top-10 -left-20 w-[500px] h-[500px] bg-indigo-500/20 rounded-full blur-[120px]"
        animate={{ x: [0, 80, 0], y: [0, 40, 0] }}
        transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-violet-600/15 rounded-full blur-[140px]"
      ...
      <ParallaxLayer speed={0.1} className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-32 right-20 w-96 h-96 bg-indigo-400/10 rounded-full blur-3xl" />
      </ParallaxLayer>
      <ParallaxLayer speed={0.2} className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/3 w-48 h-48 bg-brand-200/5 rounded-full blur-2xl" />
      </ParallaxLayer>
